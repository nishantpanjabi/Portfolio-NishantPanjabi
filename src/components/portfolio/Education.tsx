...
// Simple palette icon replacement
            ...
            {education.map((item, i) => (
              ...
              >
                ...
                <div className="absolute left-0 top-3 flex items-center justify-center w-9 h-9">
                  <div
                    className={`timeline-node ${item.current ? "timeline-node-ping" : ""}`}
                    style={{
                      background: item.color === "accent" ? "hsl(var(--accent))" : item.color === "primary" ? "hsl(var(--primary))" : "hsl(var(--muted))",
                  ...
                  {item.current && (
                    <div
                      className="absolute timeline-node opacity-50"
                      style={{
                        background: item.color === "accent" ? "hsl(var(--accent))" : "hsl(var(--primary))",
                ...
                <div className="glass-card p-6 hover:border-primary/20 transition-colors duration-300">
                  ...
                  <p
                    ...
                    style={{ color: item.color === "accent" ? "hsl(var(--accent))" : "hsl(var(--primary))" }}
                  >
                    {item.degree}
                  </p>
                  {item.details.length > 0 && (
                    <div className="flex flex-wrap gap-3">
                      {item.details.map((d) => (
                        <div key={d.label} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-background/50 border border-border">
                          <span className="text-xs text-muted-foreground">{d.label}:</span>
                          <span className="text-xs font-mono font-semibold text-foreground tabular-nums">{d.value}</span>
                        </div>
                      ))}
                    </div>
                  )}
